...
def finetune_timepoint(timepoint,adata,celltype_mapping):
    ...
    loss_fn = nn.CrossEntropyLoss(weight=None).to('cuda')
    trigger_times = 0
    max_acc = 0.0
    model.train()
    for i in tqdm(range(1, EPOCHS + 1)):
        running_loss = 0.0
        cum_acc = 0.0
        for index, (data, labels) in tqdm(enumerate(train_loader),total = len(train_loader), dynamic_ncols=True, leave=True,position=0):
            index += 1
            data, labels = data.to('cuda'), labels.to('cuda')

            logits,_ = model(data, output_attentions = True)
            labels = labels.long()  # 转换为 float32
            ...
            softmax = nn.Softmax(dim=-1)
            final = softmax(logits)
            final = final.argmax(dim=-1)  # 取出预测类别索引
            pred_num = labels.size(0)  # 批次大小
            correct_num = (final == labels).sum().item()  # 计算预测正确的数量
            cum_acc += correct_num / pred_num  # 计算当前批次的准确率
        epoch_loss = running_loss / index
        epoch_acc = 100 * cum_acc / index
        print(f'    ==  Epoch: {i} | Training Loss: {epoch_loss:.6f} | Accuracy: {epoch_acc:6.4f}%  ==')
        scheduler.step()
        ...
        with torch.no_grad():
                for index, (data_v, labels_v) in enumerate(val_loader):
                    index += 1
                    data_v, labels_v = data_v.to('cuda'), labels_v.to('cuda')
                    ...
                    predictions += final.to('cpu')
                    truths += labels_v.to('cpu')
                ...
                print(f'    ==  Epoch: {i} | Validation Loss: {val_loss:.6f}| Validation Acc: {cur_acc:.6f} | F1 Score: {f1:.6f}  ==')
                if(cur_acc>max_acc):
                    max_acc = cur_acc
                    trigger_times = 0
                    torch.save(model,'./ckpts/'+timepoint+'/best_model.pth')
                else:
                    trigger_times += 1
