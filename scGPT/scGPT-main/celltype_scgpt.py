...
from scgpt import SubsetsBatchSampler, test, train, evaluate
...
print(hyperparameter_defaults)  # 不用wandb可视化训练过程
# %%
...
if dataset_name == "PBMC_10K":
    ...
    # adata.obs["str_labels"]: 这个字段通常存储细胞类型（例如 T 细胞、B 细胞、巨噬细胞等）。将 str_labels 转换为 Pandas 的 分类数据类型（category），节省内存并加速处理。
    adata.var = adata.var.set_index("gene_symbols") # 将 gene_symbols 设为 adata.var 的索引，使得基因可以更方便地按名称查询。
    data_is_raw = True
...
# 将 "celltype" 列的值替换为对应的整数值
adata.obs["celltype"] = adata.obs["celltype"].cat.codes
# {0: 'Nb2', 1: 'gut', 2: 'neural', 3: 'epidermal', 4: 'muscle', 5: 'pharynx', 6: 'protonephridia', 7: 'cathepsin_cells', 8: 'parenchymal'}
# 打印转换的字典
print(celltype_mapping)
# adata.obs["str_labels"]: 这个字段通常存储细胞类型（例如 T 细胞、B 细胞、巨噬细胞等）。将 str_labels 转换为 Pandas 的 分类数据类型（category），节省内存并加速处理。
adata.var = adata.var.set_index("smesg") # 将 smesg 设为 adata.var 的索引，使得基因可以更方便地按名称查询。
data_is_raw = True
# %%
adata.var["gene_name"] = adata.var.index.tolist()
# 将 'orig.ident' 中的唯一值映射为一个整数
batch_id_mapping = {batch: idx for idx, batch in enumerate(adata.obs['timepoint'].unique())}
# 使用映射将 'batch_id' 填充为整数类型
adata.obs['batch_id'] = adata.obs['timepoint'].map(batch_id_mapping).astype(int)
...
if hyperparameter_defaults['load_model'] is not None:
    ...
    vocab_file = model_dir / "vocab2.json"
...
preprocessor(adata, batch_key= None) # 数据量不多，不用分批
...
#
#         print( "valid/mse:", total_loss / total_num,"valid/mre:",total_error / total_num,"valid/dab:",total_dab / total_num, "valid/sum_mse_dab:",(total_loss + hyperparameter_defaults['dab_weight'] * total_dab)/ total_num,"epoch:", epoch)
#
...
def eval_testdata(
    ...
    if "cls" in include_types:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=hyperparameter_defaults['amp']):
...
for epoch in range(1, hyperparameter_defaults['epochs'] + 1):
    ...
    if epoch % hyperparameter_defaults['save_eval_interval'] == 0 or epoch == hyperparameter_defaults['epochs']:
        ...
        torch.save(best_model, save_dir / f"model_e{best_model_epoch}.pt")
        # eval on testdata
    ...
    scheduler.step()
# save the best model
torch.save(best_model, save_dir / "best_model.pt") # 预训练完成

predictions, celltypes_labels, results = test(best_model, loader=valid_loader, gene_ids=gene_ids, vocab=vocab, config = config, device=device, logger=logger)
print(results)
